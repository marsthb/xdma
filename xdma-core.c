...
MODULE_PARM_DESC(enable_credit_mp, "Set 1 to enable credit feature, default is 0 (no credit control)");
...
static bool eth_enable = true;
module_param(eth_enable, bool, 0644);
MODULE_PARM_DESC(eth_enable, "Enable ethernet or not (default: true)");
...
extern int xgenet_init(struct xdma_dev * lro);
extern void xgenet_cleanup(void);
extern void eth_tx_cbk(void *data);
extern void eth_rx_cbk(struct xdma_engine *engine, int length, int head);
...
static int transfer_build(struct xdma_transfer *transfer, u64 ep_addr,
		...
		int userspace);
static struct xdma_transfer *rx_transfer_create(struct xdma_dev *lro,
		const char *start, size_t cnt, u64 ep_addr, int dir_to_dev,
		int non_incr_addr, int force_new_desc, int userspace, void *ptr_user_data);
...
//	struct xdma_transfer *transfer);
static ssize_t transfer_tx_data(struct xdma_engine *engine, char *transfer_addr,
		ssize_t remaining, loff_t *pos, int seq, int userspace, void *ptr_user_data);
...
static ssize_t eth_sgdma_read(struct xdma_engine *engine);
...
static void engine_start_mode_config(struct xdma_engine *engine)
{
	...
	w |= (u32)XDMA_CTRL_IE_DESC_STOPPED;
	w |= (u32)XDMA_CTRL_IE_DESC_COMPLETED;
	w |= (u32)XDMA_CTRL_IE_INVALID_LENGTH;
	w |= (u32)XDMA_CTRL_IE_IDLE_STOPPED;
#endif
...
static int engine_ring_process(struct xdma_engine *engine)
{
	...
	while (result[engine->rx_tail].status && !engine->rx_overrun) {
		...
		if (engine->rx_tail == engine->rx_head) {
			dbg_tfr("engine_ring_process(): overrun\n");
		...
		struct xdma_transfer *transfer)
{
	...
	if (transfer->last_in_request) {
		dbg_tfr("Freeing last transfer %p\n", transfer);
		transfer_destroy(engine->lro, transfer);
...
/* this is H2C, send packet, by zkh */
static int engine_service(struct xdma_engine *engine, int desc_writeback)
{
 ...
 * @dev_id pointer to xdma_dev
 ...
 no use msi-x, no use this function, by zkh
 */
static irqreturn_t xdma_user_irq(int irq, void *dev_id)
{
...
#ifdef CHAIN_MULTIPLE_TRANSFERS
static void chain_transfers(struct xdma_engine *engine,
	struct xdma_transfer *transfer)
{
 ...
 * 
 * TX and RX both use this function
 */
static int transfer_queue(struct xdma_engine *engine,
		struct xdma_transfer *transfer)
{
		...
		int dir_to_dev, int channel)
{
	...
	/* we don't use msi-x mode ,by zkh */
	if (lro->msix_enabled) {
		...
		struct xdma_transfer *transfer)
{
	...
	/* callback to eth process, add by zkh */
	if(transfer->dir_to_dev && transfer->skb)
	{
		eth_tx_cbk(transfer->skb);
	}
...
static struct xdma_transfer *tx_transfer_create(struct xdma_dev *lro,
		const char *start, size_t cnt, u64 ep_addr, int dir_to_dev,
		...
		void *ptr_user_data)
{
	...
	transfer = kzalloc(sizeof(struct xdma_transfer), GFP_ATOMIC);

	if (!transfer)
		return NULL;

	/* remember skb we sending, by zkh */
	transfer->skb = ptr_user_data;
		...
		void *ptr_user_data)
{
	...
	/* create virtual memory mapper */
	transfer->sgm = sg_create_mapper_rx(cnt);
	BUG_ON(!transfer->sgm);
	transfer->userspace = userspace;
...
static ssize_t transfer_tx_data(struct xdma_engine *engine, char *transfer_addr,
		ssize_t remaining, loff_t *pos, int seq, int userspace, 
		void *ptr_user_data)
{
//	int rc;
	ssize_t res = 0;
	...
	transfer = tx_transfer_create(lro, transfer_addr, transfer_len,
		*pos, engine->dir_to_dev, engine->non_incr_addr, 0, userspace, 
		ptr_user_data);
...
ssize_t eth_sgdma_write(struct xdma_dev *lro, char  *buf,
		size_t count, loff_t *pos, void *ptr_user_data)
{
	...
	res = transfer_tx_data(engine, (char *)buf, count, pos, seq, 0, ptr_user_data);
	dbg_tfr("seq:%d eth_sgdma_write() return=%lld.\n", seq, (s64)res);
...
/* add by zkh */
static int copy_cyclic_to_skb(struct xdma_engine *engine, int pkt_length,	int head)
{
	BUG_ON(!engine);

	eth_rx_cbk(engine, pkt_length, head);
...
static int complete_cyclic(struct xdma_engine *engine, char __user *buf)
{
	...
	/* already hold the spin lock, by zkh */
//	spin_lock(&engine->lock);
...
//		rc = copy_cyclic_to_user(engine, pkt_length, head, buf);
		rc = copy_cyclic_to_skb(engine, pkt_length, head);
		engine->rx_overrun = 0; 
...
/* add by zkh */
ssize_t eth_sgdma_read(struct xdma_engine *engine)
{
//	int rc = 0;
	int rc_len = 0;
	struct xdma_transfer *transfer;
...
//		if (rc)
//			return rc;
		rc_len += complete_cyclic(engine, 0);
...
int cyclic_transfer_setup(struct xdma_engine *engine)
{
	...
	engine->rx_transfer_cyclic = rx_transfer_create(lro, engine->rx_buffer,
		RX_BUF_SIZE, 0, engine->dir_to_dev, 0, 1, 0, NULL);
	if (engine->rx_buffer == NULL) {
		dbg_tfr("rx_transfer_create(%d) failed\n", RX_BUF_SIZE);
		rc = -EINVAL;
		goto fail_transfer;
	}
...
/* zkh, our xdma hardware does not support msi-x. this function is no use. */
static int msix_irq_setup(struct xdma_dev *lro)
{
	int i;
	int rc;
...
*/
	if(eth_enable) {
		rc = xgenet_init(lro);
		if(rc){
			printk(KERN_DEBUG "Failed xgenet_init\n");
			goto rmv_interface;
		}else {
			printk(KERN_DEBUG "xdma ethernet init successfully\n");
		}
	}
