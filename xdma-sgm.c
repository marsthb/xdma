 ...
 *
 */
struct sg_mapping_t *sg_create_mapper_rx(unsigned long max_len)
{
...
struct sg_mapping_t *sg_create_mapper(unsigned long max_len)
{
	...
	if (max_len == 0)
		return NULL;
	/* allocate bookkeeping */
	sgm = kcalloc(1, sizeof(struct sg_mapping_t), GFP_ATOMIC);
	if (sgm == NULL)
		return NULL;
	/* upper bound of pages */
	sgm->max_pages = max_len / PAGE_SIZE + 2;
	/* allocate an array of struct page pointers */
	sgm->pages = kcalloc(sgm->max_pages, sizeof(*sgm->pages), GFP_ATOMIC);
	if (sgm->pages == NULL) {
		kfree(sgm);
		return NULL;
	}
	pr_debug("Allocated %lu bytes for page pointer array for %d pages @0x%p.\n",
		sgm->max_pages * sizeof(*sgm->pages), sgm->max_pages, sgm->pages);
	/* allocate a scatter gather list */
	sgm->sgl = kcalloc(sgm->max_pages, sizeof(struct scatterlist), GFP_ATOMIC);
	if (sgm->sgl == NULL) {
		kfree(sgm->pages);
		kfree(sgm);
		return NULL;
...
int sgm_kernel_pages(struct sg_mapping_t *sgm, const char *start, size_t count, int to_user)
{
	...
	for (i = 0; i < nr_pages; i++, virt += PAGE_SIZE) {
		...
		if(is_vmalloc_addr(virt)) {
			...
			pr_debug("vmalloc_to_page 0x%llx\n", (unsigned long long)virt);
		}
		else if(virt_addr_valid(virt)) {
			pages[i] = virt_to_page(virt);
			pr_debug("virt_to_page 0x%llx\n", (unsigned long long)virt);
		}
		else {
			pages[i] = NULL;
		}
